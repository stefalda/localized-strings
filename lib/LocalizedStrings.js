...
export default class LocalizedStrings {
  ...
  formatString(str, ...valuesForPlaceholders) {
    ...
    const ref = input.split(placeholderReferenceRegex).filter(textPart => !!textPart).map(textPart => {
    ...
    return ref.split(placeholderReplaceRegex).filter(textPart => !!textPart).map(textPart => {
